...
def export_filterbank(path: str) -> None:
    ...
    with open(path, "w") as f:
        json.dump({"sr": SR, "n_fft": N_FFT, "hop": HOP, "n_mels": N_MELS,
                   "filterbank": MEL_FB.tolist()}, f)
...
def wav_to_logmel(y: np.ndarray) -> np.ndarray:
    ...
    stft = librosa.stft(y, n_fft=N_FFT, hop_length=HOP, win_length=N_FFT, window="hann", center=True)
